...
def eval(model, valid_dt, config, class_name, evaluation_step=200):
    ...
    for val_step, (images, t_bbox, t_class) in enumerate(valid_dt):
        ...
        # Log the predictions
        if config.log:
            valid_log(images, t_bbox, t_class, m_outputs, config, val_step, config.global_step,
                      class_name, evaluation_step=evaluation_step, prefix="train/")
        # Log the metrics
        ...
        if val_step % 10 == 0:
            ...
            elapsed = time.time() - t
            print(
                f"Validation step: [{val_step}], \t ce: [{log['label_cost']:.2f}] \t giou : [{log['giou_loss']:.2f}] \t l1 : [{log['l1_loss']:.2f}] \t time : [{elapsed:.2f}]")
