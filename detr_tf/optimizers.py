...
def setup_optimizers(model, config):
    ...
    transformers_optimizer = tf.keras.optimizers.Adam(
        learning_rate=get_transformers_learning_rate, clipnorm=config.gradient_norm_clipping)
...
def gather_gradient(model, optimizers, total_loss, tape, config, log):
    ...
    transformers_gradients = gradients[len(optimizers["backbone_variables"]):len(
        optimizers["backbone_variables"])+len(optimizers["transformers_variables"])]
    ...
    if getattr(config, train_part_name):
        ...
        if gradient_aggregate is not None:
            ...
            optimizers[gradient_name] = [
                (gradient + n_gradient) if n_gradient is not None else None for gradient,
                n_gradient in zip(optimizers[gradient_name],
                                  gradients)]
        else:
