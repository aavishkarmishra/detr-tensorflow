...
def tf_send_batch_log_to_wandb(
        images, target_bbox, target_class, m_outputs: dict, config, class_name=[],
        step=None, prefix=""):
    ...
    for b in range(predicted_bbox.shape[0]):
        ...
        elem_m_outputs = {key: m_outputs[key][b: b + 1]
                          if(m_outputs[key] is not None and not isinstance(m_outputs[key],
                                                                           list)) else m_outputs[key]
        ...
        np_func_params = {
            "image": image, "p_bbox": np.array(predicted_bbox),
...
def train_log(images, t_bbox, t_class, m_outputs: dict, config, step, class_name=[], prefix="train/"):
    ...
    if step % 100 == 0:
        tf_send_batch_log_to_wandb(images, t_bbox, t_class, m_outputs, config,
                                   class_name=class_name, step=step, prefix=prefix)


def valid_log(
        images, t_bbox, t_class, m_outputs: dict, config, step, global_step, class_name=[],
        evaluation_step=200, prefix="train/"):

    # Set the number of class
    WandbSender.init_ap_data(nb_class=len(class_name))
    map_list = compute_map_on_batch(
        images, t_bbox, t_class, m_outputs, config, class_name=class_name, step=global_step,
        send=(step + 1 == evaluation_step),
        prefix="val/")

    if step == 0:
        tf_send_batch_log_to_wandb(images, t_bbox, t_class, m_outputs, config,
                                   class_name=class_name, step=global_step, prefix="val/")
