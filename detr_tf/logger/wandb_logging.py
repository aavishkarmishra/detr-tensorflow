...
class WandbSender(object):
    ...
    def compute_map(p_bbox: np.array, p_labels: np.array, p_scores: np.array, t_bbox: np.array, t_labels: np.array,
        ...
        try:
            # Compyute
            cal_map(p_bbox, p_labels, p_scores, p_mask, t_bbox, t_labels,
                    t_mask, WandbSender.AP_DATA, WandbSender.IOU_THRESHOLDS)
    ...
    def get_wandb_bbox_mask_image(self, image: np.array, bbox: np.array, labels: np.array, masks=None, scores=None,
                                  class_name=[]) -> Tuple[list, np.array]:
        """
        ...
        for b in range(len(bbox)):
            ...
            image_mask = cv2.rectangle(image_mask, (int(x1*width), int(y1*height)),
                                       (int(x2*width), int(y2*height)), (1, 1, 0), 3)
    ...
    def send_proto_sample(
            proto_map: np.array, proto_sample: np.array, proto_targets: np.array, step: np.array, prefix=""):
        """
        For some reason, autograph is trying to understand what I'm doing here. With some failure. 
        Thus, @tf.autograph.experimental.do_not_convert() is used to prevent autograph to scan this method.
