...
from detr_tf.bbox import xcycwh_to_yx_min_yx_max
...
def eval_model(model, config, class_names, valid_dt):
    ...
    for images, target_bbox, target_class in valid_dt:
        ...
        # Compute map
        cal_map(p_bbox, p_labels, p_scores,  np.zeros((138, 138, len(p_bbox))), np.array(t_bbox),
                np.array(t_class), np.zeros((138, 138, len(t_bbox))), ap_data, iou_thresholds)
